...
function createGitHubClient(token) {
  ...
  return {
    async listComments(org2, repo2, prNumber2) {
    ...
    async createComment(org2, repo2, prNumber2, body) {
    ...
    async updateComment(org2, repo2, commentId, body) {
...
// ../../packages/config/src/ci-config.ts
function parseCiConfig(input) {
  const raw = input ?? {};
  ...
  if (typeof publishDirectory !== "object" || publishDirectory === null || Array.isArray(publishDirectory) || !Object.values(publishDirectory).every((v) => typeof v === "string")) {
    throw new Error(
...
function buildCommitUrl(params) {
  const base = params.baseUrl.replace(/\/+$/, "");
  return `${base}/${params.org}/${params.repo}/commit/${params.sha}/${params.packageName}`;
}

// src/build.ts
async function runBuild(config, repoRoot2, executor) {
  if (config.installCommand !== false) {
    await executor.run(`npm run ${config.installCommand}`, { cwd: repoRoot2 });
  }
  if (config.buildCommand !== false) {
    await executor.run(`npm run ${config.buildCommand}`, { cwd: repoRoot2 });
  }
}

// src/comment.ts
var MARKER = "<!-- draftpkg -->";
async function postPrComment(options, github) {
...
var CONFIG_PATHS = [".draftpkg/ci.json", ".codesandbox/ci.json"];
async function loadConfig(repoRoot2) {
  for (const configPath of CONFIG_PATHS) {
    const fullPath = path.join(repoRoot2, configPath);
    try {
...
async function packPackages(packages, publishDirectory, repoRoot2, executor) {
  const results = [];
  for (const pkg of packages) {
    const publishDir = publishDirectory[pkg.name];
    const packDir = publishDir ? path2.join(repoRoot2, publishDir) : pkg.path;
    const { stdout } = await executor.run("npm pack", { cwd: packDir });
    const tarballName = stdout.trim();
    results.push({
      packageName: pkg.name,
      tarballPath: path2.join(packDir, tarballName)
    });
  }
  return results;
}

// src/resolve-packages.ts
import fs2 from "node:fs/promises";
import path3 from "node:path";
async function resolvePackages(repoRoot2, patterns) {
  const packages = [];
  for (const pattern of patterns) {
    if (isGlob(pattern)) {
      const dirs = await expandGlob(repoRoot2, pattern);
      ...
      const fullPath = path3.join(repoRoot2, pattern);
...
async function readPackageName(dir) {
  try {
    const content = await fs2.readFile(path3.join(dir, "package.json"), "utf-8");
...
async function expandGlob(repoRoot2, pattern) {
  const parts = pattern.split("*");
  if (parts.length !== 2) {
    throw new Error(`Unsupported glob pattern: ${pattern}`);
  }
  const prefix = parts[0];
  const baseDir = path3.join(repoRoot2, prefix);
  let entries;
  try {
    entries = await fs2.readdir(baseDir);
  } catch {
    return [];
  }
  const results = [];
  for (const entry of entries) {
    const fullPath = path3.join(baseDir, entry);
...
async function uploadPackages(packages, options, httpClient) {
  for (const pkg of packages) {
    const url = buildCommitUrl({
      baseUrl: options.workerUrl,
      org: options.org,
    ...
    const body = new Uint8Array(await fs3.readFile(pkg.tarballPath));
...
async function run(options) {
  const { repoRoot: repoRoot2 } = options;
  const executor = createExecutor();
  const httpClient = createHttpClient();
  console.log(`Loading config from ${repoRoot2}\u2026`);
  const config = await loadConfig(repoRoot2);
  console.log(`Resolving packages matching: ${config.packages.join(", ")}\u2026`);
  const packages = await resolvePackages(repoRoot2, config.packages);
  console.log(
    `Found ${packages.length} package(s): ${packages.map((p) => p.name).join(", ")}`
  );
  await runBuild(config, repoRoot2, executor);
  console.log("Packing\u2026");
...
async function getPrNumber() {
  ...
  try {
    const event = JSON.parse(await fs4.readFile(eventPath, "utf-8"));
    return event.pull_request?.number ?? void 0;
  } catch {
    return void 0;
  }
}
var repoRoot = process.env.GITHUB_WORKSPACE ?? process.cwd();
var sha = process.env.INPUT_SHA || process.env.GITHUB_SHA;
if (!sha) {
  throw new Error(
    "Missing SHA: provide the `sha` input or run in a GitHub Actions environment"
  );
}
var [org, repo] = (process.env.GITHUB_REPOSITORY ?? required("repository")).split("/");
var githubToken = process.env.GITHUB_TOKEN;
var prNumber = await getPrNumber();
run({
  ...
  sha,
  prNumber,
  github: githubToken ? createGitHubClient(githubToken) : void 0
}).catch((error) => {
  console.error(error);
  process.exit(1);
});
